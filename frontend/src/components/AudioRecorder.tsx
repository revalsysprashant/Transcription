        ...
        try {
            ...
            recording.ondataavailable = (event) => {
                ...
                if (limits && bytes > limits.max_size_bytes) {
                    ...
                    if (mounted.current)
                        setError(
                            "Recording exceeds the upload size limit. Please record a shorter clip.",
                        );
            ...
            recording.onstop = () => {
                ...
                if (failed) return;
                const type =
                    recording.mimeType || chunks[0]?.type || "audio/webm";
            ...
            if (mounted.current) {
                setError(
                    "Cannot access the microphone. Check your browser microphone permission.",
                );
                setActive(false);
                onBusyChange(false);
            }
        ...
        <div className="mt-5 rounded-lg border border-zinc-200 p-4">
            ...
            <div className="mt-3 flex gap-3">
                <button
                    ...
                    onClick={start}
                    className="rounded-lg bg-zinc-950 px-4 py-2 text-sm text-white disabled:opacity-50"
                >
                    Start recording
                </button>
                <button
            ...
            {active && (
                <p role="status" className="mt-2 text-sm">
                    Microphone active or awaiting permission…
                </p>
            )}
            {preview && (
                <audio
            ...
            {preview && (
                <p className="mt-2 text-sm">
                    Listen, then choose Upload and transcribe below.
                </p>
            )}
            {error && (
